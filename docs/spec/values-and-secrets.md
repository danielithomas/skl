...
```bash
skl values sync --from ../acme-skill-values
skl values sync --from git@github.com:acme-org/skill-values.git --profile default
```
...
```yaml
# acme-skill-values/default.values.yaml
consulting_company: Acme Consulting
